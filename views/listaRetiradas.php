...
<body>
    <div id="back_ground"
        ...
        <aside class="fixed z-10 w-14 h-full flex justify-start items-start overflow-hidden flex-col self-start transition-all hover:w-56 bg-cyan-700 drop-shadow-2xl shadow-2xl border-r-[3px] border-solid border-cyan-800">
            ...
            <a href="retiradas.php" class="w-56 h-14 flex justify-center items-center text-white cursor-pointer transition-all bg-cyan-600 shadow-2xl hover:bg-cyan-600 hover:shadow-2xl">
        ...
        <section id="screen_top" class="w-full h-20 grid grid-cols-3 grid-rows-1">
            ...
            <div class="flex justify-center items-center">
                <p class="text-white text-2xl font-bold drop-shadow-2xl shadow-2xl">Lista de Retiradas</p>
            </div>
            <div></div>
        </section>
            ...
            <main class="mt-10 w-[60%] flex justify-center items-center flex-col gap-y-2.5 gap-x-2">
                ...
                <div class="w-full flex <?php echo($livro_counter > 0 ? "justify-between" : "justify-start")?> items-center">
                    <div class="py-1 px-5 flex justify-center items-center gap-x-1 bg-white font-medium rounded-sm drop-shadow-2xl shadow-2xl">
                        ...
                        <p class="font-bold <?php echo($livro_counter == 0 ? "text-red-500" : "")?>"><?php echo($livro_counter > 0 ? $livro_counter : "Esgotado")?></p>
                    </div>
                    <?php if ($livro_counter > 0) {?>
                        <a href="retirada.php" class="py-1 px-5 bg-white font-medium cursor-pointer rounded-sm transition-all drop-shadow-2xl shadow-2xl hover:bg-[rgb(220,220,220)]">Inserir Retirada</a>
                    <?php }?>
                </div>
                <table class="w-full rounded-sm drop-shadow-2xl shadow-2xl">
                    <tr class="h-9 bg-white">
                        <td class="w-[5%] text-center font-bold">ID</td>
                        <td class="w-[12%] pl-5 font-bold">Aluno</td>
                        <td class="w-[32%] text-center font-bold">Livro</td>
                        <td class="w-[12%] text-center font-bold">Retirada</td>
                        <td class="w-[12%] text-center font-bold">Devolucao</td>
                    ...
                    foreach ($retiradas as $retirada) {
                        ...
                        <tr class="h-9 bg-[rgba(255,255,255,<?php echo ($count % 2 == 0 ? "0.5" : "0.3");?>)] transition-all hover:bg-[rgba(255,255,255,0.8)]">
                            <td class="text-center font-bold"><?php echo ($retirada->getId())?></td>
                            <td class="pl-5 text-sm"><?php echo ($retirada->getAluno_id())?></td>
                            <td class="text-center text-sm"><?php echo ($retirada->getLivro_id())?></td>
                            <td class="text-center text-sm"><?php echo ($retirada->getRetirada())?></td>
                            <td class="text-center text-sm"><?php echo ($retirada->getDevolucao())?></td>
                            <td class="flex justify-center items-center text-center">
                                <a href="excluirRetirada.php?id=<?php echo $retirada->getId();?>" class="grow h-9 flex justify-center items-center hover:text-red-500 transition-all cursor-pointer drop-shadow-2xl shadow-2xl"><i class="fa-solid fa-trash"></i></a>
                            </td>
                        </tr>
                    <?php } ?>
                </table>
